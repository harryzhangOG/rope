...
def segment_online(base_img, raw_img, seq_no):
    ...
    canvas[:400, :120, :] = 0
    canvas[:200, :, :] = 0
    canvas[200:300, 430:, :]=0

    cv2.imwrite('physical_whip_eval_redo/seg_%04d.png'%(seq_no), canvas)

def record_webcam(seq_no):
    pic_name = 'whip_eval_%04d.png'%(seq_no)
    d2r = pi / 180
    # Expert policy: midpoint config
    if not os.path.exists("./physical_whip_eval_redo"):
        os.makedirs('./physical_whip_eval_redo')
    ...
    cv2.imwrite(os.path.join('./physical_whip_eval_redo', pic_name), vis)
    base_img = cv2.imread('./physical_exp_datagen/base_img_eval_redo_knock_new.png')
    ...
    state_dict = torch.load('resnet_ur5_model_physical_redo.pth', map_location=device)['model_state_dict']
    net50.load_state_dict(state_dict)
    net50.resnet_mean.to(device)

    in_image = Image.open('physical_whip_eval_redo/seg_%04d.png'%(seq_no)).convert("RGB")
    ...
    os.chdir('../ur5_go')
    os.system('./build/Debug/ur5_go -h 172.22.22.3 -r 0.1 -t 0.032 ur5_taut.dat')
    os.system('./build/Debug/ur5_go -h 172.22.22.3 -r 0.1 -t 0.032 ur5_taut_reset.dat')
    ...
    if inp in yesChoice:
        ...
        os.chdir('../rope')
        np.save('physical_whip_eval_redo/a.npy', pred_mid_config)
        return
