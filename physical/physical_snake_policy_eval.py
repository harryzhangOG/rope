...
def segment_online(base_img, raw_img, seq_no):
    ...
    cv2.imwrite('physical_snake_eval/seg_%04d.png'%(seq_no), canvas)

def record_webcam(seq_no):
    pic_name = 'snake_eval_%04d.png'%(seq_no)
    d2r = pi / 180
    # Expert policy: midpoint config
    if not os.path.exists("./physical_snake_eval"):
        os.makedirs('./physical_snake_eval')
    ...
    cv2.imwrite(os.path.join('./physical_snake_eval', pic_name), vis)
    base_img = cv2.imread('./physical_exp_datagen/base_img_eval_snake_new.png')
    ...
    state_dict = torch.load('resnet_ur5_model_physical_snake_2000.pth', map_location=device)['model_state_dict']
    net50.load_state_dict(state_dict)
    net50.resnet_mean.to(device)

    in_image = Image.open('physical_snake_eval/seg_%04d.png'%(seq_no)).convert("RGB")
    ...
    # Take the first half of the motion

    dat_main_datagen(pred_mid_config[:6])
    ...
    if inp in yesChoice:
        ...
        dat_main_datagen(pred_mid_config[6:])
        os.chdir('../ur5_go')
        os.system('./build/Debug/ur5_go -h 172.22.22.3 -r 0.8 -t 0.032 ur5_reset_datagen.dat')
        os.chdir('../rope')
        return
