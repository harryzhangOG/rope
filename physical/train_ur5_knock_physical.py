...
def train_1000():
    ...
    # Load data
    path = os.path.join(os.path.join(os.getcwd(), 'whip_knock_dataset'))
    holdout = 150
    ...
    torch.save({'model_state_dict': net50.state_dict(),
               ...
               }, 'resnet_ur5_model_physical_knock.pth')
    if not os.path.exists("./results_knock_ur5_physical"):
        os.makedirs('./results_knock_ur5_physical')
    save_dir = os.path.join(os.getcwd(), 'results_knock_ur5_physical')
...
def train_2000():
    ...
    # ResNet 50
    net50 = DistModel(device, 6)
    state_dict = torch.load('resnet_ur5_model_physical_knock.pth')['model_state_dict']
    net50.load_state_dict(state_dict)
    net50.resnet_mean.to(device)
    ...
    torch.save({'model_state_dict': net50.state_dict(),
                'optimizer_state_dict': optimizer.state_dict()
               }, 'resnet_ur5_model_physical_knock_2000.pth')
    save_dir = os.path.join(os.getcwd(), 'results_knock_ur5_physical')
